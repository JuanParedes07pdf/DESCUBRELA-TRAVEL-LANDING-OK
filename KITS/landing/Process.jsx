const Process = () => {
  ...
  React.useEffect(() => {
    if (!window.gsap || !window.ScrollTrigger || !fillRef.current) return;
    const gsap = window.gsap;
    const tween = gsap.fromTo(fillRef.current,
      { scaleX: 0 },
      {
        scaleX: 1,
        ease: 'power2.out',
  ...
  return (
    <section id="proceso" style={{ padding: '96px 0', background: '#0A2473', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="container" style={{ position: 'relative', zIndex: 1 }}>
        ...
        {/* Rail wrapper: cards + connector behind */}
        <div ref={railRef} className="process-rail" style={{ position: 'relative' }}>
          {/* Track (gray base) — sits at icon vertical center: card pad-top 28 + icon radius 26 = 54 */}
          <div className="process-track" aria-hidden="true" style={{
            position: 'absolute',
            top: 54,
            left: 'calc(12.5% + 0px)',
            right: 'calc(12.5% + 0px)',
          ...
          <div ref={fillRef} className="process-fill" aria-hidden="true" style={{
            position: 'absolute',
            top: 54,
            left: 'calc(12.5% + 0px)',
            right: 'calc(12.5% + 0px)',
            height: 2,
            background: 'linear-gradient(90deg, #25D366 0%, #164DF2 50%, #F5A623 100%)',
            borderRadius: 2,
            transform: 'scaleX(0)',
          ...
          <div className="grid-4" style={{ position: 'relative', zIndex: 1 }}>
            {steps.map((s, i) => (
              <div key={i} className="fade-in process-step-wrap">
                <div data-gsap="process-step" style={{
                  ...
                  textAlign: 'center', transition: 'background 0.25s, transform 0.25s, border-color 0.25s',
                  cursor: 'default',
                }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.12)'; e.currentTarget.style.transform = 'translateY(-4px)'; e.currentTarget.style.borderColor = 'rgba(245,166,35,0.35)'; }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.07)'; e.currentTarget.style.transform = 'translateY(0)'; e.currentTarget.style.borderColor = 'rgba(255,255,255,0.10)'; }}>
                  <div style={{
                    width: 52, height: 52, borderRadius: '50%', background: '#164DF2',
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    margin: '0 auto 16px', boxShadow: '0 8px 24px rgba(22,77,242,0.4), 0 0 0 6px #0A2473',
                    fontSize: 20, position: 'relative', zIndex: 2,
                  }}>
                    {s.icon}
                  </div>
                  <div style={{ fontSize: 10, fontWeight: 800, color: 'rgba(245,166,35,0.85)', letterSpacing: '0.14em', marginBottom: 8 }}>{s.num}</div>
